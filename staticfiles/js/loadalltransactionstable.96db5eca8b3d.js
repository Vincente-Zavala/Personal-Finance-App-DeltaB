    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                console.log("originalCategoryId", categoryTd.dataset.originalCategoryId);
                console.log("availableOptions", Array.from(categorySelect.options).map(o => ({value: o.value, hidden: o.hidden})));
                console.log("can we restore?", categorySelect.querySelector(`option[value="${categoryTd.dataset.originalCategoryId}"]`));
                ...
                console.log(
                    "BUILD CHECK",
                    tr.querySelector('select[name="category"]')
                  );
    ...
    document.addEventListener("click", (e) => {
        ...
        if (cancelBtn) {
            ...
            if (categorySelect && categoryDisplay) {
                // 1. Unhide & enable all options first
                Array.from(categorySelect.options).forEach(option => {
                    option.hidden = false;
                    option.disabled = false;
                });

                // 2. Restore original value from dataset
                categorySelect.value = categoryTd.dataset.originalCategoryId;

                // 3. Restore display text from dataset
