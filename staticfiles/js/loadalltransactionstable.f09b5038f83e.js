document.addEventListener("DOMContentLoaded", () => {
    ...
    filterForm.addEventListener("submit", e => {
        ...
        filterForm.querySelectorAll("input, select").forEach(input => {
            if (input.type === "checkbox" || input.type === "radio") {
                ...
                const selected = Array.from(input.options).some(opt => opt.selected);
                if (!selected) input.disabled = true;
            } else if (!input.value) {
                input.disabled = true;
            }
        });
        ...
        filterForm.querySelectorAll("input, select").forEach(input => input.disabled = false);
        ...
        .then(res => res.json())
        .then(data => {
            renderAllTransactions(data.transactions || [], data.appliedfilters || [])
    ...
    // -------------------------
    function renderAllTransactions(transactions, appliedFilters = []) {
