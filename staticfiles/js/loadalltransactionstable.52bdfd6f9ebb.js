    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                const dateObj = new Date(tx.date); // tx.date in YYYY-MM-DD
                ...
                dateDisplay.className = "tx-display";
                dateDisplay.textContent = formattedDate; // Dec 1, 2025

                // Flatpickr input
                const dateInput = document.createElement("input");
                dateInput.type = "text";
                dateInput.value = tx.date;  // keep ISO for flatpickr
                dateInput.className = "form-control form-control-sm tx-input d-none flatpickr";
                dateInput.name = "date";
                ...
                flatpickr(dateInput, {
                    dateFormat: "Y-m-d",       // actual value
                    defaultDate: tx.date,
                    allowInput: true,
                    altInput: true,             // nicer display in input
                    altFormat: "M j, Y",        // Dec 1, 2025
                    onChange: function(selectedDates, dateStr) {
                        // Update display span while editing
                        if (selectedDates[0]) {
                            const friendlyDate = selectedDates[0].toLocaleDateString('en-US', {
                                year: 'numeric',
                                month: 'short',
                                day: 'numeric'
                            });
...
document.addEventListener("click", (e) => {
    ...
    if (cancelBtn) {
        ...
        // --- Restore TYPE ---
        const typeSelect = row.querySelector('select[name="type"]');
        const typeDisplay = row.querySelector('td:nth-child(2) .tx-display');
        ...
        const dateDisplay = row.querySelector('td:nth-child(2) .tx-display'); // adjust selector if needed
        
        if (dateInput && dateDisplay) {
            const originalDate = new Date(dateInput.dataset.originalValue);
            dateDisplay.textContent = originalDate.toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'short',
                day: 'numeric'
            });
        ...
        if (amountInput && amountDisplay) {
            ...
            amountInput.classList.add("d-none"); // hide input
            amountDisplay.classList.remove("d-none"); // show display
        }
