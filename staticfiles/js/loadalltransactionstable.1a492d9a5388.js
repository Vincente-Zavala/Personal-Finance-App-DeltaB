    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                dateDisplay.className = "tx-display fw-semibold";
                dateDisplay.textContent = tx.formatted_date;
            
                const dateInput = document.createElement("input");
                dateInput.type = "date";
                dateInput.value = tx.formatted_date;
...
document.addEventListener("click", (e) => {
    ...
    if (editBtn) {
        ...
        if (dateInput) {
            row.dataset.originalDate = dateInput.value;
        }
    ...
    if (cancelBtn) {
        ...
        // --- RESTORE DATE ---
        const dateInput = row.querySelector('input[name="date"]');
        const dateDisplay = row.querySelector('td:nth-child(1) .tx-display'); // adjust index if needed
        if (dateInput && dateDisplay) {
            dateInput.value = row.dataset.originalDate || dateInput.value;
            dateInput.classList.add("d-none");
            dateDisplay.textContent = row.dataset.originalDate || dateDisplay.textContent;
