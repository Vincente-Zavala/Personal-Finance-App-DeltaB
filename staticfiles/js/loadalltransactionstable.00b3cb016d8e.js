...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.addEventListener("click", (e) => {
        ...
        //         const originalOption = categorySelect.querySelector(`option[value="${categoryTd.dataset.originalCategoryId}"]`);
        //         if (originalOption) {
        //             categorySelect.value = categoryTd.dataset.originalCategoryId;
        //         } else {
        //             // If the original option is not there (rare, maybe deleted), just reset
        //             categorySelect.value = "";
        //         }
            
        //         // Restore display
        //         categoryDisplay.textContent = categoryTd.dataset.originalCategoryName;
        //     }
        ...
        if (cancelBtn) {
            ...
            if (categorySelect && categoryDisplay) {
                // 1. Unhide & enable all options temporarily
                Array.from(categorySelect.options).forEach(option => {
                    option.hidden = false;
                    option.disabled = false;
                });
                ...
                const originalCategoryId = categoryTd.dataset.originalCategoryId;
                const originalOption = categorySelect.querySelector(`option[value="${originalCategoryId}"]`);

                if (originalOption) {
                    categorySelect.value = originalCategoryId;
                    categoryDisplay.textContent = originalOption.textContent; // restore display text
                } else {
                    // fallback if option no longer exists
                    categorySelect.value = "";
                    categoryDisplay.textContent = categoryTd.dataset.originalCategoryName;
                }
            }
            ...
            // -------------------
            const typeSelect = row.querySelector('select[name="type"]');
            const typeDisplay = row.querySelector('td:nth-child(2) .tx-display'); // adjust if needed

            if (typeSelect && typeSelect.dataset.originalValue) {
                typeSelect.value = typeSelect.dataset.originalValue;
                if (typeDisplay) typeDisplay.textContent = typeSelect.dataset.originalValue;
            }
