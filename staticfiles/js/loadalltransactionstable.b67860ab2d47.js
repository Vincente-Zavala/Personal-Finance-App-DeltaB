document.addEventListener("DOMContentLoaded", () => {
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="text-end fw-semibold text-primary">$${tx.amount}
    ...
    toggleEditBtn.addEventListener("click", () => {
        ...
        // Show/hide inputs and original text
        document.querySelectorAll(".tx-input").forEach(input => {
            const span = input.previousSibling; // the $${tx.amount} text node
            if (editMode) {
                input.classList.remove("d-none");
                input.previousSibling.nodeValue = ""; // hide text
            } else {
                input.classList.add("d-none");
                input.previousSibling.nodeValue = `$${input.value}`; // restore text
            }
        });
