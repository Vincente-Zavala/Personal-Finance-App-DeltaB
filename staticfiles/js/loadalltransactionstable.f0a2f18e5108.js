...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (editBtn) {
            ...
            console.log("categorySelect 1", categorySelect)
            row.dataset.originalCategory = categorySelect.value;
            console.log("row.dataset.originalCategory", row.dataset.originalCategory)

            typeSelect.addEventListener("change", () => {
                const selectedType = typeSelect.value;
                console.log("selectedType", selectedType)
                const currentCategoryOriginal = categorySelect.dataset.originalValue;
                console.log("currentCategoryOriginal 1", currentCategoryOriginal)

                // Rebuild category select based on new type
                const newCategorySelect = buildCategorySelect({ id: categorySelect.dataset.transactionId, category_name: categorySelect.options[categorySelect.selectedIndex]?.text || "" }, selectedType);
                console.log("newCategorySelect", newCategorySelect)
                console.log("currentCategoryOriginal 2", currentCategoryOriginal)

                newCategorySelect.value = row.dataset.originalCategory;

                // Preserve original value
                newCategorySelect.dataset.originalValue = currentCategoryOriginal;
                console.log("newCategorySelect.dataset.originalValue", newCategorySelect.dataset.originalValue)
                console.log("currentCategoryOriginal 3", currentCategoryOriginal)
                newCategorySelect.name = "category";
                newCategorySelect.classList.add("tx-input");

                categorySelect.replaceWith(newCategorySelect);
                console.log("categorySelect", categorySelect)
                console.log("currentCategoryOriginal 4", currentCategoryOriginal)
                console.log("row.dataset.originalCategory 2", row.dataset.originalCategory)
            });

            console.log("row.dataset.originalCategory 3", row.dataset.originalCategory)
        ...
        if (cancelBtn) {
            console.log("Within if cancelbtn")
            const row = cancelBtn.closest(".transaction-row");
            console.log("row.dataset.originalCategory 4", row.dataset.originalCategory)
            if (!row) return;
            ...
            //     console.log("input.dataset.originalValue", input.dataset.originalValue)
            //     if (!input) return;
            //     if (input.tagName === "SELECT") {
            //         if (input.dataset.originalValue != null) {
            //             input.value = input.dataset.originalValue;
            //         }
            //         console.log("selectOption", input.value)
            ...
            //     }
            //     console.log("Originals", input.value)
            // });

            // CATEGORY (always correct)
            const categorySelect = row.querySelector('select[name="category"]');
            const categoryDisplay = row.querySelector('td:nth-child(4) .tx-display');
            console.log("categoryDisplay", categoryDisplay)
            
            console.log("categorySelect 2", categorySelect)

            if (categorySelect && categoryDisplay) {
                categorySelect.value = row.dataset.originalCategory;
            
                const selectedOption = categorySelect.options[categorySelect.selectedIndex];
                categoryDisplay.textContent = selectedOption ? selectedOption.text : "";
            }
            // if (categorySelect) {
            //     categorySelect.value = row.dataset.originalCategory;
            //     console.log("Originals category", categorySelect.value)
            // }
   

            // TYPE
            const typeSelect = row.querySelector('select[name="type"]');
            if (typeSelect && typeSelect.dataset.originalValue) {
                typeSelect.value = typeSelect.dataset.originalValue;
                console.log("Originals type", typeSelect.value)
            }
            ...
            if (amountInput && amountDisplay) {
                amountInput.value = amountDisplay.textContent.replace(/[^0-9.-]+/g, "");
            }
