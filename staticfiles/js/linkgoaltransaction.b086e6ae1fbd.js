document.querySelectorAll('input[name="selectedtransactions"]').forEach(checkbox => {
    checkbox.addEventListener('change', async function() {
        ...
        try {
            ...
            // ---- Update disabled checkboxes and "×" for ALL goals dynamically ----
            document.querySelectorAll('input[name="selectedtransactions"]').forEach(cb => {
                ...
                // If this transaction is linked to another goal and not checked for this goal
                if (data.other_goals_map[tid] && !cb.checked) {
                    cb.disabled = true;
