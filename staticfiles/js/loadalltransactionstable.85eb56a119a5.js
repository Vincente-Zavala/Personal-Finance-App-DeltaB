...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            const container = document.getElementById("allTransactionsCards");
            container.innerHTML = ""; // Clear old cards
            
            data.transactions.forEach(tx => {
              const card = document.createElement("div");
              card.classList.add("transaction-card", "p-3", "mb-3", "rounded", "shadow-sm", "bg-white");
              card.dataset.editing = "false";
            
              // Build inner content flexibly
              card.innerHTML = `
                <div class="d-flex align-items-center mb-2 gap-3">
                  ...
                  <div>
                    <button class="btn btn-sm p-0 tx-link" data-tx-id="${tx.id}" type="button" title="View linked transaction">
                      <i class="fa fa-chevron-right text-primary"></i>
                    </button>
                  </div>
                  <div class="fw-semibold tx-date">${tx.formatted_date}</div>
                </div>
            
                <div class="d-flex justify-content-between flex-wrap gap-3">
                  <div class="type-category d-flex align-items-center gap-2">
                    <i class="fa-solid ${categoryTypeStyles[tx.type_name.toLowerCase().replace(/\s+/g, "")]?.icon || ''}" 
                       style="color: ${categoryTypeStyles[tx.type_name.toLowerCase().replace(/\s+/g, "")]?.color || '#000'}"></i>
                    <span style="color: ${categoryTypeStyles[tx.type_name.toLowerCase().replace(/\s+/g, "")]?.color || '#000'}">${tx.type_name}</span>
                    <span class="badge fw-semibold" style="background-color: ${categoryTypeStyles[tx.type_name.toLowerCase().replace(/\s+/g, "")]?.color || '#ccc'}">${tx.category_name}</span>
                  </div>
            
                  <div class="note flex-grow-1 text-truncate">${tx.user_note || ''}</div>
            
                  <div class="account text-nowrap">${tx.account_display || ''}</div>
            
                  <div class="amount fw-semibold text-primary text-end">$${tx.amount}</div>
                </div>
              `;
