...
document.addEventListener("DOMContentLoaded", () => {
    ...
    toggleEditBtn.addEventListener("click", () => {
        ...
        document.querySelectorAll(".transaction-row").forEach(tr => {
            // Get the transaction ID (assuming it's available in the row)
            const txId = tr.getAttribute("data-tx-id");

            const typeSelect = tr.querySelector(".tx-input[type='select']"); // Type select input
            const typeDisplay = tr.querySelector(".tx-display"); // Type display span

            const categorySelect = tr.querySelector(".tx-input[type='select']"); // Category select input
            const categoryDisplay = tr.querySelector(".tx-display"); // Category display span

            // Store original values when entering edit mode
            if (editMode) {
                originalTypeValues[txId] = typeDisplay.textContent;
                originalCategoryValues[txId] = categoryDisplay.textContent;
            }
            ...
            inputs.forEach((input, i) => {
                ...
                if (editMode) {
                    ...
                    if (originalTypeValues[txId] && originalCategoryValues[txId]) {
                        typeDisplay.textContent = originalTypeValues[txId];
                        categoryDisplay.textContent = originalCategoryValues[txId];
                    }
                }
            });
        });
