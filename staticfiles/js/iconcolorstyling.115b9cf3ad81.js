...
const limitedStyles = {
  income: categoryTypeStyles.income,
  expense: categoryTypeStyles.expense,
};

function applyIncomeExpenseRowStyles() {
  document.querySelectorAll(".transaction-row").forEach(row => {
    console.log("ROW TYPE VALUE:", `"${row.dataset.type}"`);
    const type = row.dataset.type?.toLowerCase();

    // Only style income/expense
    if (!["income", "expense"].includes(type)) return;

    const style = limitedStyles[type];
    if (!style) return;
    ...
    // Style the text
    if (amountCell) {
      amountCell.style.color = style.color;
      amountCell.style.fontWeight = "600";
    }
  });
}

document.addEventListener("DOMContentLoaded", applyIncomeExpenseRowStyles);
