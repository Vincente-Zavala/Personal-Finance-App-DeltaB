...
// Expenses Doughnut
document.addEventListener("DOMContentLoaded", () => {
    const raw = document.getElementById("charts-data").textContent.trim();
    const chartsData = JSON.parse(raw);

    // 👇 pick a specific one
    const chartObj = chartsData.find(obj => obj.title === "Expense Breakdown");
    // or just use chartsData[0] if you know its position
    ...
    const colors = ["#eb1616", "#ff4040", "#ff8080"]; // or use makeColors()
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: chartObj.title || "Expense Breakdown",
                },
                legend: { position: "top" },
            },
        },
