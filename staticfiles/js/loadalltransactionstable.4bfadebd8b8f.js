...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.addEventListener("click", (e) => {
        ...
        //         // Set the original category value to the new category select
        //         newCategorySelect.dataset.originalValue = currentCategoryOriginal;

        //         // Set the new category select as a replacement for the old one
        //         categorySelect.replaceWith(newCategorySelect);
                
        //         // Now, the original category value is preserved
        ...
        //     // Revert category select to its original value
        //     const categorySelect = row.querySelector('select[name="category"]');
        //     if (categorySelect && categorySelect.dataset.originalValue) {
        ...
        if (cancelBtn) {
            ...
            const typeSelect = row.querySelector('select[name="type"]');
            const categoryTd = row.querySelector("td:nth-child(4)"); // category column
            const categoryDisplay = categoryTd.querySelector(".tx-display");
        
            // 🔁 Restore TYPE
            typeSelect.value = row.dataset.originalType;
        
            // 🔁 Rebuild CATEGORY SELECT from scratch
            const newCategorySelect = buildCategorySelect(
                {
                    id: typeSelect.dataset.transactionId,
                    category_name: categoryDisplay.textContent
                },
                row.dataset.originalType
            );
        
            newCategorySelect.classList.add("tx-input", "d-none");
            newCategorySelect.name = "category";
        
            // Replace mutated select
            const oldCategorySelect = categoryTd.querySelector("select");
            oldCategorySelect.replaceWith(newCategorySelect);
        
            // Exit edit mode
            row.querySelectorAll(".tx-input").forEach(i => i.classList.add("d-none"));
            row.querySelectorAll(".tx-display").forEach(d => d.classList.remove("d-none"));
            row.querySelectorAll(".tx-cancel, .tx-check").forEach(b => b.classList.add("d-none"));
