...
function buildCategorySelect(tx) {
    ...
    CATEGORY_TYPES.forEach(ct => {
        ...
        ct.categories.forEach(cat => {
            ...
            option.dataset.categorytype = ct.type; // key for 2-account logic
...
// -------------------------
function buildAccountSelect(tx) {
    const select = document.createElement("select");
    select.className = "form-select form-select-sm rounded-pill category-select";
...
function buildDestinationSelect(tx) {
    ...
    // populate accounts if you have accounts array from template or API
    if (window.ACCOUNTS) {
        window.ACCOUNTS.forEach(acc => {
            const option = document.createElement("option");
            option.value = acc.id;
            option.textContent = `${acc.institution_name} - ${acc.name}`;
