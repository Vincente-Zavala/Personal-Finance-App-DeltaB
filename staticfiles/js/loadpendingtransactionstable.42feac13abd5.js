document.addEventListener("DOMContentLoaded", () => {
    if (typeof TRANSACTIONS_API_URL === "undefined") {
    ...
    fetch(TRANSACTIONS_API_URL, { credentials: "same-origin" })
        .then(res => {
        ...
        .then(data => {
            ...
            data.results.forEach(tx => {
                ...
                tr.innerHTML = `
                    <td class="editcol" hidden><input class="form-check-input" type="checkbox" name="selectedtransactions" value="{{ transaction.id }}"></td>
                    ...
                    <td>
                        <select 
                            ...
                            {% for category in categorytype.displaycategories %}
                                <option value="{{ category.id }}" data-categorytype="{{ categorytype.name|lower }}">{{ category.name }}</option>
                            {% endfor %}
                            </optgroup>
                            {% else %}
                            <optgroup label="{{ categorytype.name }}">
                  ...
                  <td>
                    <select 
                        ...
                        {% for account in accounts %}
                            <option value="{{ account.id }}">{{ account.institution.name }} - {{ account.name }}</option>
                        {% endfor %}
                    </select>
                  </td>
                `;
