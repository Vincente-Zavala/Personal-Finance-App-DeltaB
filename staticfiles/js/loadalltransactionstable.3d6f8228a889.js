    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                // DATE with optional chevron
                const dateTd = document.createElement("td");
                dateTd.classList.add("date-col"); // optional class for styling

                // Container for chevron + date
                const dateContainer = document.createElement("div");
                dateContainer.className = "d-flex align-items-center gap-2";

                // Add chevron button if linkable
                if (linkable) {
                    const chevronDiv = document.createElement("div");
                    chevronDiv.className = "tx-chevron"; // for CSS styling if needed

                    const chevronButton = document.createElement("button");
                    chevronButton.className = "btn btn-sm p-0 tx-link";
                    chevronButton.dataset.txId = tx.id;
                    chevronButton.type = "button";
                    chevronButton.title = "View linked transaction";

                    const chevronIcon = document.createElement("i");
                    chevronIcon.className = "fa fa-chevron-right text-primary";

                    chevronButton.appendChild(chevronIcon);
                    chevronDiv.appendChild(chevronButton);
                    dateContainer.appendChild(chevronDiv);
                }
                ...
                // Add date input (hidden for editing)
                const dateInput = document.createElement("input");
                dateInput.type = "text"; // required for flatpickr
                dateInput.value = tx.date_iso;
                dateInput.className = "form-control form-control-sm all-tx-input d-none tx-date-input";
                ...
                // Append display and input to the container
                dateContainer.append(dateDisplay, dateInput);

                // Append the container to the TD
                dateTd.appendChild(dateContainer);

                // Finally, append the TD to the row
                tr.appendChild(dateTd);
