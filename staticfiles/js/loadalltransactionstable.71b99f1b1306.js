        ...
        if (editBtn) {
            ...
            typeSelect.addEventListener("change", () => {
                ...
                Array.from(categorySelect.options).forEach(opt => {
                    if (!opt.value) return; // keep the "Select Category..." option
                    opt.hidden = opt.dataset.type !== selectedType;
                    opt.hidden = opt.dataset.type.toLowerCase()
                });
            
                // If current selected category is not valid for the new type, reset
