document.addEventListener("DOMContentLoaded", () => {
    ...
    let editMode = false; // tracks if user clicked Edit

    // Fetch metadata for dropdowns
    fetch("/api/transaction-metadata/") 
        .then(res => res.json())
        .then(data => {
            types = data.types;
    ...
    toggleEditBtn.addEventListener("click", () => {
        editMode = !editMode;
        renderAllTransactions(currentTransactions, appliedFiltersGlobal, oneAccountGlobal, types, categories, accounts);
        document.getElementById("submittransactions").style.display = editMode ? "inline-block" : "none";
        document.getElementById("deletetransactions").hidden = !editMode;
    });

    const filterForm = document.getElementById("filterTransactionsForm");
    let currentTransactions = [];
    let appliedFiltersGlobal = [];
    let oneAccountGlobal = false;
    ...
    filterForm.addEventListener("submit", e => {
        ...
        .then(data => {
            ...
            appliedFiltersGlobal = data.appliedfilters || [];
            oneAccountGlobal = data.one_account;
            renderAllTransactions(currentTransactions, appliedFiltersGlobal, oneAccountGlobal, types, categories, accounts);
    ...
    function renderAllTransactions(transactions, appliedFilters = [], oneAccount = false, types = [], categories = [], accounts = []) {
        ...
        transactions.forEach(tx => {
            ...
            tr.innerHTML = `
                ...
                <td>${editMode ? `<input type="month" class="form-control form-control-sm editable" data-field="date" value="${tx.date.slice(0,7)}">` : tx.formatted_date}</td>
                <td>${editMode ? `<select class="form-select form-select-sm editable" data-field="type">${typeOptions}</select>` : tx.type_name}</td>
                <td>${editMode ? `<select class="form-select form-select-sm editable" data-field="category">${categoryOptions}</select>` : (tx.category_name || "")}</td>
                <td>${editMode ? `<div contenteditable="true" class="editable" data-field="note">${tx.note || ""}</div>` : (tx.note || "")}</td>
                <td>${editMode ? `<select class="form-select form-select-sm editable" data-field="account">${accountOptions}</select>` : tx.account_display}</td>
                <td>${editMode ? `<div contenteditable="true" class="editable text-end" data-field="amount">${tx.amount}</div>` : `$${tx.amount}`}</td>
