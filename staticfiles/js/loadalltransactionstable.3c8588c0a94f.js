    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                const dateObj = new Date(tx.date); // tx.date can be YYYY-MM-DD or ISO string
                ...
                flatpickr(dateInput, {
                    dateFormat: "Y-m-d",       // actual value saved
                    defaultDate: isoDate,       // must be ISO
                    allowInput: true,
                    altInput: true,             // nicer display for user
                    altFormat: "M j, Y",        // Dec 1, 2025
                    onChange: function(selectedDates, dateStr) {
                        if (selectedDates[0]) {
                            dateDisplay.textContent = selectedDates[0].toLocaleDateString('en-US', {
                                year: 'numeric',
                                month: 'short',
                                day: 'numeric'
                            });
