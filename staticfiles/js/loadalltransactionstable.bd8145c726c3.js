let CATEGORY_TYPES = [];  // will store type + categories

document.addEventListener("DOMContentLoaded", () => {
    if (typeof ALL_TRANSACTIONS_API_URL === "undefined") return;

    fetch("/api/categories/", { credentials: "same-origin" })
        .then(res => res.json())
        .then(data => {
            CATEGORY_TYPES = data.category_types || [];
            return fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" });
        })
        .then(res => res.json())
...
function createDropdown(name, options, selectedValue) {
    ...
    select.className = "form-select form-select-sm tx-input d-none"; // reuse tx-input class
    select.name = name;

    options.forEach(opt => {
        const option = document.createElement("option");
        option.value = opt.name; // or opt.id if you prefer IDs
    ...
    fetch(ALL_TRANSACTIONS_API_URL, { credentials: "same-origin" })
        ...
        .then(data => {
            ...
            data.transactions.forEach(tx => {
                ...
                const typeObj = CATEGORY_TYPES.find(ct => ct.name === tx.type_name) || { categories: [] };
                ...
                // Append dropdowns for edit mode
                const typeCell = tr.querySelectorAll(".category-cell")[0];
                const categoryCell = tr.querySelectorAll(".category-cell")[1];

                const typeDropdown = createDropdown(`tx_type_${tx.id}`, CATEGORY_TYPES, tx.type_name);
                const categoryDropdown = createDropdown(`tx_category_${tx.id}`, typeObj.categories, tx.category_name);
