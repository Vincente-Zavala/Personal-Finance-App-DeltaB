...
// -------------------------
function buildCategorySelect(tx, typeFilter = null) {
    const select = document.createElement("select");
    ...
    CATEGORY_TYPES.forEach(ct => {
        if (typeFilter && ct.name !== typeFilter) return; // only show categories for selected type
...
document.addEventListener("change", function(e) {
    ...
    // Type changed → update category options
    if (target.classList.contains("type-select")) {
        ...
        // Rebuild category dropdown for the selected type
